"""
Single-call LTX 2 image modular text-to-image runner.

This uses the custom LTX2ImageAutoBlocks metadata. The same blocks can run
text2image or image2image; this runner forces the text2image path by not
passing an input image.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="LTX 2 image modular text-to-image runner.")
...
def main():
    ...
    prompt_tag = "_".join(args.prompt.split()[:6]).lower()
    run_slug = f"ltx2_image_modular_t2i_{args.width}x{args.height}_steps{args.steps}_seed{args.seed}_{prompt_tag}"
    ...
    t0 = tracker.step_start(f"Modular Text2Image at {args.width}x{args.height}")
    ...
    record_event("modular_text2image_call", time.time() - event_t0)
    ...
    del pipe
    flush()
    tracker.step_end(f"Modular Text2Image at {args.width}x{args.height}", t0)
