...
from diffusers.models.transformers.transformer_ltx2 import LTX2Attention, LTX2AudioVideoAttnProcessor
...
class LTX2ImageTextConnectors(ModelMixin, PeftAdapterMixin, ConfigMixin):
    """
    Text connector stack used by the pruned LTX-2 Image pipeline to process the packed text encoder hidden states
    only for the visual (spatial) stream.
    """
    ...
    def forward(
        ...
        """
        Given per-layer text encoder hidden_states, extracts features and runs visual connector to get text
        embeddings for the LTX-2.X Image models.
        ...
        Args:
            ...
            attention_mask (`torch.Tensor` of shape `(batch_size, seq_len)`):
                ...
                positions.
            padding_side (`str`, *optional*, defaults to `"left"`):
                The padding side used by the text encoder's text encoder. Defaults to `"left"`.
            scale_factor (`int`, *optional*, defaults to `8`):
                Scale factor for masked mean/range normalization.
        """
        ...
        if self.config.per_modality_projections:
            ...
            video_text_emb_proj = self.text_proj_in(norm_text_encoder_hidden_states)
        ...
        return video_text_embedding, binary_attn_mask.squeeze(-1)
