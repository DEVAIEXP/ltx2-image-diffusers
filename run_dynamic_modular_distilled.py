...
#MODEL_PATH = env_value("DDO_RUNNER_MODEL_PATH", r"F:\model\ltx2.3-image-distilled-1.1-sdnq-int8")
print(MODEL_PATH)
...
DYNAMIC_OFFLOAD_EXECUTION_MODE = DYNAMIC_OFFLOAD_SETTINGS.config.execution_mode
DYNAMIC_OFFLOAD_PIN_CPU_MEMORY = DYNAMIC_OFFLOAD_SETTINGS.requested_pin_cpu_memory
DYNAMIC_OFFLOAD_SHOW_PROFILE = DYNAMIC_OFFLOAD_SETTINGS.config.show_profile
...
def main():
    ...
    if FAKE_PROMPT_EMBEDS:
        ...
        print(
            f"  [ddo] text_encoder: route={text_encoder_offload.route} preset={DYNAMIC_OFFLOAD_PRESET or 'none'}",
            flush=True,
        )
    ...
    def load_and_prepare_transformer(repeat_index: int):
        ...
        run_metrics["transformer_offload_route"] = transformer_offload.route
        print(
            f"  [ddo] transformer: route={transformer_offload.route} preset={DYNAMIC_OFFLOAD_PRESET or 'none'}",
            flush=True,
        )
    ...
    for prepare_repeat_index in range(TRANSFORMER_PREPARE_REPEATS):
        ...
        if prepare_repeat_index + 1 < TRANSFORMER_PREPARE_REPEATS:
            if dynamic_offload_hook is not None:
    ...
    if dynamic_offload_hook is not None and DYNAMIC_OFFLOAD_EXECUTION_MODE != "plan":
