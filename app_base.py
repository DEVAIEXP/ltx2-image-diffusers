"""
Gradio app for LTX 2.3 base T2I generation with an optional I2I pass.
...
MODEL_PATH = os.getenv("LTX_BASE_MODEL_PATH", r"elismasilva/ltx2.3-image-base")
SDNQ_MODEL_PATHS = {
    "int4": os.getenv("LTX_BASE_SDNQ_INT4_PATH", r"elismasilva/ltx2.3-image-base-sdnq-int4"),
    "int8": os.getenv("LTX_BASE_SDNQ_INT8_PATH", r"elismasilva/ltx2.3-image-base-sdnq-int8"),
}
...
DEFAULT_NEGATIVE_PROMPT = (
    ...
    "grainy texture, poor lighting, distorted proportions, unnatural anatomy, deformed features, artifacts, "
...
OUTPUT_DIR = Path(os.getenv("LTX_GRADIO_OUTPUT_DIR", "outputs/gradio_base"))
...
def encode_prompt(prompt, negative_prompt="", guidance_scale=3.0):
    ...
    flush()
    return (prompt_embeds.to(OFFLOAD_DEVICE), prompt_attention_mask.to(OFFLOAD_DEVICE), negative_prompt_embeds.to(OFFLOAD_DEVICE), negative_prompt_attention_mask.to(OFFLOAD_DEVICE))
...
def run_stage1(prompt, negative_prompt, width, height, seed, bits, num_inference_steps, guidance_scale, pag_enabled, pag_scale, pag_layers, crisp_enabled, crisp_scale, soft_enabled, soft_scale, progress=gr.Progress()):
    width, height = ensure_size(width, height)
    t0 = time.time()
    progress(0.05, desc="Encoding prompt")
    prompt_embeds, prompt_attention_mask, negative_prompt_embeds, negative_prompt_attention_mask = encode_prompt(prompt, negative_prompt, guidance_scale=float(guidance_scale))
    ...
    del prompt_embeds, prompt_attention_mask, negative_prompt_embeds, negative_prompt_attention_mask, pipe, transformer
    flush()
    ...
    return image, {"stage": "stage1", "bits": bits, "steps": int(num_inference_steps), "cfg_scale": float(guidance_scale), "loras": active_loras, "pag_enabled": pag_enabled, "pag_scale": float(pag_scale) if pag_enabled else 0.0, "pag_layers": parse_pag_layers(pag_layers) if pag_enabled else None, "elapsed_sec": round(elapsed, 2)}
...
def run_stage2(
    ...
    prompt_embeds, prompt_attention_mask, negative_prompt_embeds, negative_prompt_attention_mask = encode_prompt(normalize_prompt, negative_prompt, guidance_scale=float(guidance_scale))
    ...
    flush()
    elapsed = time.time() - t0
    return output, {"stage": "stage2", "bits": bits, "steps": int(num_inference_steps), "cfg_scale": float(guidance_scale), "loras": active_loras, "pag_enabled": pag_enabled, "pag_scale": float(pag_scale) if pag_enabled else 0.0, "pag_layers": parse_pag_layers(pag_layers) if pag_enabled else None, "elapsed_sec": round(elapsed, 2)}
...
def run_app(
    ...
    if stage2_only:
        ...
        stage1_image, metrics = run_stage1(
            ...
            guidance_scale=stage1_cfg_scale,
        ...
        logs.append(f"T2I done in {metrics['elapsed_sec']}s using {metrics['bits']} | steps: {metrics['steps']} | CFG: {metrics['cfg_scale']} | PAG: {metrics['pag_scale']} {metrics['pag_layers']}.")
    ...
    if enable_stage2 or stage2_only:
        stage2_image, metrics = run_stage2(
            ...
            bits="bf16",
            num_inference_steps=stage1_steps,
            guidance_scale=stage2_cfg_scale,
        ...
        logs.append(f"I2I done in {metrics['elapsed_sec']}s using {metrics['bits']} | steps: {metrics['steps']} | CFG: {metrics['cfg_scale']} | PAG: {metrics['pag_scale']} {metrics['pag_layers']}.")
    else:
...
with gr.Blocks(title="LTX 2.3 Image - Base Model") as demo:
    gr.Markdown("# LTX 2.3 Image - Base Model")
    gr.Markdown("T2I + optional I2I base image generation with negative prompt support and optional Enhance/Crisp LoRAs.")
    ...
    with gr.Row():
        stage1_steps = gr.Slider(label="Steps", minimum=1, maximum=60, value=28, step=1)
    with gr.Row():
    ...
    with gr.Accordion("Text-to-Image", open=True):
        with gr.Row():
            ...
            stage1_cfg_scale = gr.Slider(label="CFG Scale", minimum=1.0, maximum=8.0, value=3.0, step=0.1)
    ...
    with gr.Accordion("Image-to-Image", open=False):
        with gr.Row():
            ...
            stage2_cfg_scale = gr.Slider(label="CFG Scale", minimum=1.0, maximum=8.0, value=3.0, step=0.1)
