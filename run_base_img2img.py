"""
Standalone LTX 2.3 base bf16 image-to-image script.
...
def parse_args():
    parser = argparse.ArgumentParser(description="LTX 2.3 base bf16 img2img test.")
    ...
    parser.add_argument("--negative-prompt", default=DEFAULT_NEGATIVE_PROMPT, help="Negative prompt used when classifier-free guidance is active.")
    ...
    parser.add_argument("--steps", type=int, default=28, help="Base number of denoising steps before strength slicing.")
...
def make_run_slug(
    ...
    *,
    ...
    model_tag = "bf16"
    text_encoder_tag = "text_encoder_original"
...
def main():
    ...
    do_classifier_free_guidance = args.guidance_scale > 1.0 and bool(args.negative_prompt)
    ...
    run_metrics = {
        ...
        "sdnq_enabled": False,
        "sdnq_bits": None,
        "sdnq_quantized_version": None,
        "sdnq_version": None,
        "sdnq_model_path": None,
        "sdnq_text_encoder_bits": None,
    ...
    with torch.inference_mode():
        prompt_embeds, prompt_attention_mask, negative_prompt_embeds, negative_prompt_attention_mask = embeds_pipe.encode_prompt(
            prompt=args.prompt,
            negative_prompt=args.negative_prompt,
            do_classifier_free_guidance=do_classifier_free_guidance,
        )
    record_event("encode_prompt_call", time.time() - event_t0, classifier_free_guidance=do_classifier_free_guidance)
    ...
    image = pipe(
        ...
        negative_prompt_embeds=negative_prompt_embeds.to(device=DEVICE, dtype=DTYPE) if negative_prompt_embeds is not None else None,
        negative_prompt_attention_mask=negative_prompt_attention_mask.to(device=DEVICE) if negative_prompt_attention_mask is not None else None,
        width=args.width,
        height=args.height,
        num_inference_steps=args.steps,
        strength=args.strength,
    ...
    metrics_lines = [
        ...
        f"SDNQ_ENABLED: {run_metrics['sdnq_enabled']}",
        f"SDNQ_BITS: {run_metrics['sdnq_bits'] if run_metrics['sdnq_enabled'] else 'none'}",
        ...
        f"CLASSIFIER_FREE_GUIDANCE: {do_classifier_free_guidance}",
