"""
Memory-scoped LTX 2.3 distilled modular image-to-image runner.

This runner keeps the img2img behavior from the older distilled script, including
LoRA options, while loading each modular component only for the phase that needs
it. Text encoder and transformer use official Diffusers group offloading.
"""
...
MODEL_TAG = "distilled_modular_img2img_diffusers_group_offload"
...
def parse_args():
    parser = argparse.ArgumentParser(description="LTX 2.3 distilled modular img2img runner.")
    ...
    parser.add_argument("--output-dir", default="outputs/ltx_image_modular_distilled_img2img")
...
def available_lora_adapters(model) -> set[str]:
    if model is None or not hasattr(model, "get_list_adapters"):
        return set()
    adapters = model.get_list_adapters()
...
def main():
    ...
    print("Using distilled modular img2img Diffusers group offload", flush=True)
